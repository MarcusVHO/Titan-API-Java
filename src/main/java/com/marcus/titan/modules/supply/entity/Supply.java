...
import com.marcus.titan.exceptions.MaterialCantRefund;
...
public class Supply {
    ...
    public void refund() throws Exception {
        if (status == SupplyStatus.CLAIMED || status == SupplyStatus.PICKING) {
            this.status = SupplyStatus.REQUESTED;
            this.movements.refund();
            return;
        }
        throw new MaterialCantRefund();
    }

    public void pick() {
        this.status = SupplyStatus.PICKING;
        this.movements.startPicking();
    }

    public void placeInBuffer() {
        this.status = SupplyStatus.IN_BUFFER;
        this.movements.endPicking();
    }

    public void pickRefueling(Integer operatorId) {
        this.status = SupplyStatus.REFUELING_MODULE;
        this.movements.startRefueling(operatorId);
    }

    public void supply() {
        this.status = SupplyStatus.SUPPLIED;
        this.movements.endRefueling();
    }
